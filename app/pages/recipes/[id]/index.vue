<template>
  <div class="p-4 max-w-3xl mx-auto">
    <div class="flex items-center justify-between mb-4">
      <h1 class="text-2xl font-semibold">{{ recipe?.title || t('recipes.loading') }}</h1>
      <div class="flex gap-2">
        <UButton size="sm" @click="navigateTo(`/recipes/${id}/edit`)">{{ t('recipes.edit') || 'Edit' }}</UButton>
        <UButton size="sm" variant="ghost" @click="navigateTo('/')">{{ t('common.cancel') }}</UButton>
      </div>
    </div>
    ...
    <div v-else class="text-center py-8">{{ t('recipes.loading') || 'Loading...' }}</div>
  </div>
</template>
