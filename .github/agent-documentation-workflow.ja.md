...
**強制ルール**:
- コードや API の変更を PR に含める場合、必ず同一 PR で以下のいずれかを更新すること:
  - `.agent/specs/*`
  - `.agent/docs/*` または `.agent/docs/tasks/*`
  - `CHANGELOG.md`
- もし上記のいずれかをすぐに更新できない合理的な理由がある場合は、PR に `spec-exception` ラベルを付け、`.agent/docs/qa/vX.Y-qa.md` に理由と次のアクション（いつ誰が更新するか）を記載すること。
- CI ワークフロー（`Specs Check`）が PR を検出し、上記の更新がない場合は CI が失敗するようになっています。失敗した場合は QA を追加して再度 PR を更新してください。
