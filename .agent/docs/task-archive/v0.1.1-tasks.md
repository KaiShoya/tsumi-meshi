...
- **PR**: merged (`chore/specs-enforcement` → `main` on 2025-12-14`) 
...
- **Search & Filter**
  ...
  - [x] Unit tests for search behavior and edge cases
  - [x] Integration tests (API-level) for search endpoint

- **Status update**: Unit and integration tests added for `recipes` store and API endpoints. CI (lint/typecheck/tests) pass locally. Changes pushed to `main`.
