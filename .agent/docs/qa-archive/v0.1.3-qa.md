...
**Question**: When running the dev server or Vitest, Vue emits warnings like "Failed to resolve component: UFormField" and "Extraneous non-props attributes ...". How should we handle this?
...
- **CI ワークフロー修正を適用**: `.github/workflows/ci.yml` を整理・統合し、以下を反映しました。
	- 重複ワークフローの削除と統合
	- `RUN_MINIFLARE` の手動/変数制御を追加（`workflow_dispatch` 入力 + `env` デフォルト）
	- `paths-ignore` を導入し、`.agent/**` の変更のみで CI をスキップするように調整（`.github/**` はトリガー対象のまま）

- **認証テスト**: 統合テストをローカルで実行し、以下が全て通過しました。
	- `tests/integration/auth-refresh.spec.ts`
	- `tests/integration/auth-refresh-workers.spec.ts`
	- `tests/integration/auth-refresh-miniflare.spec.ts`（Miniflare 未インストール時はスキップする実装）

**決定**: Miniflare ベースの E2E はオプションとし、必要に応じて CI で有効化する（`RUN_MINIFLARE=true` をリポジトリの Actions 変数に設定）。

## 次の QA ステップ

- 1) CI を有効化した状態での CI 実行（Actions 変数 `RUN_MINIFLARE=true` を一時的に設定してジョブを確認）。
- 2) 既存の TODO を Issue に変換し、優先度・オーナーを割り当てる（タスク管理）。
- 3) Stage D（unit/integration/a11y）の自動テストを全通させるための優先修正を洗い出す。
