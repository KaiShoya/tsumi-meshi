...
- [x] **API (scaffold)**: 画像アップロード用のスケルトンAPIを `server/api/upload/image.post.ts` に追加（署名ロジックは未実装）
- [ ] **API (impl)**: 画像アップロード用の署名付きURLを生成するサーバーAPI (`/api/upload/image`) を実装する（presign）
