...
Overview
- Session model: cookie-based refresh token (long-lived) + short-lived access token used by server. Client does not persist tokens in localStorage; auth state is derived from server via `/api/v1/auth/me`.
...
- POST `/api/v1/auth/refresh` (internal, optional)
  - Request: cookie only
...
- Integration: `/api/v1/auth` endpoints using a test Workers environment or a stubbed in-memory store
- E2E: refresh flow (simulate cookie set/refresh/expiry) to ensure client-side session is correctly initialized by `/api/v1/auth/me`
...
- The `apiClient` includes `login`, `register`, `getCurrentUser` (`/api/v1/auth/me`) and `logout` helpers; `logout()` issues `POST /api/v1/auth/logout` and clears client session state.
...
- Unit tests cover happy/failure paths for login/register and `me` initialization, and integration/E2E tests validate refresh cookie rotation and session rehydration via `/api/v1/auth/me`.
